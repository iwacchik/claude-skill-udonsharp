...
- SDK 3.10.5 でデシリアライズの堅牢性が改善（`[}` のような不正な閉じ括弧を invalid 扱い・`["["]` `["{"]` のような文字列内の括弧を正しく解析・`{ "x": { "y": "{" } }` のようにネストした括弧と文字列内の括弧が混在するオブジェクトも正しくデシリアライズ）。3.10.4 以前は不正な JSON の一部を受理したり、上記のような正当な JSON に失敗したりすることがある。
